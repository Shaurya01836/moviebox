...
export function WatchlistCard({ item, onEdit, onDelete, index = 0, isPersonalCollection }: WatchlistCardProps) {
  ...
  const targetHref = isPersonalCollection
    ? `/collections/title/${item.mediaId}`
    : `/${item.mediaKind === 'tv' ? 'tv' : 'movies'}/${item.mediaId}`;
    ...
    <div className="group relative flex flex-col space-y-2.5 transition-all">
      ...
      <div className="relative block aspect-[2/3] overflow-hidden rounded-2xl bg-zinc-900">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-zinc-950/80 opacity-0 backdrop-blur-sm transition-all duration-300 group-hover:opacity-100 p-4 text-center">
          ...
          {!isPersonalCollection && (
            <Link 
              href={`/play/${item.mediaKind === 'tv' ? 'tv' : 'movie'}/${item.mediaId}`}
          ...
          {!isPersonalCollection ? (
            ...
            <div className="absolute bottom-4 left-0 right-0 flex justify-center gap-2 mt-4">
              <Link
                href={targetHref}
                className="px-4 py-1.5 rounded-full bg-white/10 text-zinc-300 hover:bg-white/20 hover:text-white transition-colors cursor-pointer backdrop-blur-md text-xs font-medium"
              >
                View Details
              </Link>
              <button
                ...
                className="flex h-7 w-7 items-center justify-center rounded-full bg-white/10 text-zinc-300 hover:bg-white/20 hover:text-white transition-colors cursor-pointer backdrop-blur-md"
                title="Edit Entry & Collections"
              >
